...
export default function PrivateRoute({ authentication }: PrivateRouteProps): React.ReactElement | null {
  ...
  if (authentication) {
    ...
    return isAuthenticated === null || isAuthenticated === undefined ? <Navigate to="/login" /> : <Outlet />;
  } else {
    // 인증이 반드시 필요 없는 페이지

    // 인증을 안햇을 경우 해당 페이지로 인증을 한 상태일 경우 main페이지로
    return isAuthenticated === null || isAuthenticated === undefined ? <Outlet /> : <Navigate to="/" />;
  }
}
